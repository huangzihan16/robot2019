...
Eigen::Matrix4d TagDetection::getRelativeTransform(double tag_size, double fx, double fy, double px, double py, double k1, double k2, double k3, double k4) const {
 ...
 // cv::Vec4f distParam(0,0,0,0);
  cv::Vec4f distParam(k1,k2,k3,k4); // all 0?
  cv::solvePnP(objPts, imgPts, cameraMatrix, distParam, rvec, tvec);  //采用p3p
...
void TagDetection::getRelativeTranslationRotation(double tag_size, double fx, double fy, double px, double py,
    ...
    double k2 = 0;
     double k3 = 0;
     double k4 = 0;                                                 
  Eigen::Matrix4d T = getRelativeTransform(tag_size, fx, fy, px, py,  k1,  k2,  k3,  k4);
