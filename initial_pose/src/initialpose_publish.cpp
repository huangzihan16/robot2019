...
initialpose_node::initialpose_node(std::string node_name) {
	...
	initialpose_pub_ = n_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 50);
}
...
bool initialpose_node::setTransform(std::string laser_topic_name) {
  ...
  try {
	...
	x_lb_ = pose_stamp.getOrigin().x();
	y_lb_ = pose_stamp.getOrigin().y();
	double yaw, pitch, roll;
	pose_stamp.getBasis().getEulerYPR(yaw, pitch, roll);
	alpha_lb_ = yaw;
	cos_alpha_lb_ = cos(yaw);
	sin_alpha_lb_ = sin(yaw);
	return true;
