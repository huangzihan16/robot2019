...
void initialpose_estimation::corner_extract() {
	...
	for (int i = 10; i < ranges_boundary_.size() - 10; i++) {
...
void initialpose_estimation::variance_calculate() {
	...
	if (type_ == LEFT) {
		for (int i = 0; i < corner_ - 9; i++) {
			...
			double delta_alpha1 = acos(-initial_pose_(0) / ranges_boundary_[i]);
		...
		for (int i = corner_ + 10; i < ranges_boundary_.size(); i++) {
			...
			double delta_alpha1 = asin(-initial_pose_(1) / ranges_boundary_[i]);
		...
		for (int i = corner_ + 10; i < ranges_boundary_.size(); i++) {
			...
			double delta_alpha1 = acos((8 - initial_pose_(0)) / ranges_boundary_[i]);
			double delta_alpha2 = -delta_alpha1;
			delta_alpha1 -= angles_boundary_[i] + initial_pose_(2);
			delta_alpha2 -= angles_boundary_[i] + initial_pose_(2);
	...
	variance_xx_ /= (corner_ - 9);
	variance_yy_ /= (ranges_boundary_.size() - corner_ - 10);
	variance_aa_ /= (ranges_boundary_.size() - 19);
	covariance_xa_ /= (corner_ - 9);
	covariance_ya_ /= (ranges_boundary_.size() - corner_ - 10);
}
