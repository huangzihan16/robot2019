...
class SearchBehavior {
 public:
  SearchBehavior(ChassisExecutor* &chassis_executor, GimbalExecutor* &gimbal_executor,
  ...
  void Run() {
    ...
    if (executor_state != BehaviorState::RUNNING) {
      ...
      ROS_INFO("%s,%d","search_count_",blackboard_->search_count_);
    }
  }

  void Cancel() {
    chassis_executor_->Cancel();
    blackboard_->search_count_ = 0;
